...
prometheus:
  ingress:
    ...
    path: /
    hosts:
      - prometheus-${local.domain_suffix}
    tls:
    - hosts:
      - prometheus-${local.domain_suffix}
  prometheusSpec:
...
grafana:
  ...
  deploymentStrategy:
    type: Recreate
  adminPassword: ${local.kube_prometheus_stack.enabled ? random_string.grafana_password[0].result : "test123"}
  serviceAccount:
    annotations:
  ...
  ingress:
    ...
    path: /
    hosts:
      - grafana-${local.domain_suffix}
    tls:
    - hosts:
      - grafana-${local.domain_suffix}
  env:
    # all values must be quoted
    GF_SERVER_ROOT_URL: "https://grafana-${local.domain_suffix}"
...
alertmanager:
  ...
  ingress:
    ...
    path: /
    hosts:
      - alertmanager-${local.domain_suffix}
    tls:
    - hosts:
      - alertmanager-${local.domain_suffix}
  alertmanagerSpec:
...
output "grafana_domain_name" {
  value       = local.kube_prometheus_stack.enabled ? "grafana-${domain.suffix}" : null
  description = "Grafana dashboards address"
}

output "alertmanager_domain_name" {
  value       = local.kube_prometheus_stack.enabled ? "alertmanager-${domain.suffix}" : null
  description = "Alertmanager ui address"
}

output "prometheus_domain_name" {
  value       = local.kube_prometheus_stack.enabled ? "prometheus-${domain.suffix}" : null
  description = "Prometheus ui address"
}

output "grafana_admin_password" {
  value       = local.kube_prometheus_stack.enabled ? random_string.grafana_password[0].result : null
  sensitive   = true
